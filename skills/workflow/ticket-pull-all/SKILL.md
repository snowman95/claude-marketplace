...
description: >-
  JQL 1회 호출로 여러 Jira 이슈를 일괄 pull해 .claude/tickets/ 에 저장한다. 이슈마다 getJiraIssue 반복 금지.
...
3. 각 이슈마다 ticket-pull과 동일하게 `.claude/tickets/{fixVersion}/{JIRA-KEY}.md` 생성·갱신.
4. 완료 후 절대 경로 목록·이슈 수 요약, ticket-workflow 다음 단계 안내.

> fixVersion·저장 경로·파일 구조 상세 → ticket-pull `REFERENCE.md`  
